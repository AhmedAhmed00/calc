...
function calculateWorkData(hoursWorked) {
...
document.getElementById("calculateBtn").addEventListener("click", () => {
  const input = parseFloat(document.getElementById("hoursInput").value);
  ...
  const result = calculateWorkData(input);

  resultDiv.innerHTML = `
    ✅ <strong>Total hours worked:</strong> ${result.hoursWorked.toFixed(
      2
    )} hrs<br>
    📈 <strong>Expected hours until today:</strong> ${result.expectedHoursUntilToday.toFixed(
      2
    )} hrs<br>
    ${
      result.hourDifference >= 0
        ? `🟢 <strong>You are ${result.hourDifference.toFixed(
            2
          )} hours ahead of schedule</strong><br>`
        : `🔴 <strong>You are ${Math.abs(result.hourDifference).toFixed(
            2
          )} hours behind schedule</strong><br>`
    }
    ⏳ <strong>Remaining hours this month:</strong> ${result.remainingHours.toFixed(
      2
    )} hrs<br>
    📅 <strong>Workdays passed:</strong> ${result.passedWorkingDays}<br>
    📆 <strong>Workdays remaining:</strong> ${result.remainingWorkingDays}<br>
    📊 <strong>Month progress (workdays):</strong> ${result.monthProgressPercentage.toFixed(
      1
    )}%<br>
    ⚙️ <strong>Work progress (hours):</strong> ${result.completionPercentage.toFixed(
      1
    )}%<br>
